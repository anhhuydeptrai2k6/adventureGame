...
public class Hoat_Anh_Nhan_Vat {
        ...
        switch (trangthai){
            ...
            case DUNGYEN:
                if (timedoiframe > 0.05f){
                    frames[0] = (frames[0] + 1) % dungyen.length;
                    timedoiframe = 0;
                }
                break;
            case CHAY:
                if (timedoiframe > 0.05f){
                    frames[1] = (frames[1] + 1) % chay.length;
                    timedoiframe = 0;
                }
                break;
            case NHAYKEP:
                if (timedoiframe > 0.05f){
                    frames[2] = (frames[2] + 1) % nhaykep.length;
                    timedoiframe = 0;
                }
                break;
        }
    ...
    public void ve(Batch batch, float x, float y, boolean flipX, TRANGTHAI trangthai){
        ...
        switch (trangthai){
            case DUNGYEN:
                veNhanVat = dungyen[frames[0]];
                break;
            case CHAY:
                veNhanVat = chay[frames[1]];
                break;
            case NHAY:
                veNhanVat = nhay;
                break;
            case NHAYKEP:
                veNhanVat = nhaykep[frames[2]];
                break;
            case ROI:
                veNhanVat = roi;
                break;
